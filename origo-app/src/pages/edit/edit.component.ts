...
export class EditComponent implements OnInit {
  ...
  onLoadCliente() {
    ...
    this.data.nascimento = new Date(this.data.nascimento).toISOString();
  ...
  onSubmit() {
    if (this.clienteForm.valid.valueOf()) { 
      ...
      this.ApiService.editCliente(cliente)
      .subscribe(r => {         
          this.process = false;
          this.dialogRef.close();
          this.refresh()
          this.snackBar.open('Cliente Successfully Updated!', '', { duration: 4000 });       
      }, e => {
